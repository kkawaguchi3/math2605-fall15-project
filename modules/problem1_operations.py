...
def solve_lu_b(A, b):
    ...
    # x = U\(L\(P'*b))
    # first find (L\(P'*b))
    forw = forward_sub(L, (np.dot(A.T, b)))
    # then find U\forw
    x = forw
...
def backward_sub(A, b_orig):
    ...
    for i in range(n - 2, 0, -1):
        print(i)
        x[i] = (b[i] - np.dot(A[i, i:].flatten(), x[i:])) / A[i, i]
        print(x)
    return x
...
def save_lu_results():
    lu_data = part_d_lu()
    save_to_csv('../data/lu_error_data.csv', lu_data)


def save_hh_results():
    hh_data = part_d_qr()
    save_to_csv('../data/hh_error_data.csv', hh_data)


def save_givens_results():
    givens_data = part_d_qr_givens()
    save_to_csv('../data/givens_error_data.csv', givens_data)
